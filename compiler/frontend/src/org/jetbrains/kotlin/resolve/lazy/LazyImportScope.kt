...
public class LazyImportScope(private val resolveSession: ResolveSession,
    ...
    private fun <D : DeclarationDescriptor> selectSingleFromImports(
        ...
        fun compute(): D? {
            val imports = importsProvider.getImports(name, selectImportsMode) // it returns either only one explicit directive or all directives with '*' so we can correctly use "singleOrNull"
            ...
            return imports.stream()
                    .map { directive -> descriptorSelector.get(getImportScope(directive, lookupMode), name) }
                    .filterNotNull()
                    .singleOrNull()
        }
