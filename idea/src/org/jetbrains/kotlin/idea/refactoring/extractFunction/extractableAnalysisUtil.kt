...
private fun getCommonSuccessorIfAny(instructions: List<Instruction>): Instruction? {
    val visitor = object: InstructionVisitorWithResult<Boolean>() {
        var target: Instruction? = null

        override fun visitInstructionWithNext(instruction: InstructionWithNext): Boolean {
            return when (instruction) {
                is LocalFunctionDeclarationInstruction,
    ...
    if (!instructions.flatMap { it.nextInstructions }.all { it.accept(visitor) }) return null
    return visitor.target ?: instructions.firstOrNull()?.owner?.getSinkInstruction()
}
...
private fun ExtractionData.analyzeControlFlow(
    ...
    if (typeOfDefaultFlow.isMeaningful()) {
        ...
        if (getCommonSuccessorIfAny(valuedReturnExits) == null) return multipleExitsError
    ...
    if (jumpExits.isNotEmpty()) {
        val jumpTarget = getCommonSuccessorIfAny(jumpExits)
        if (jumpTarget == null) return multipleExitsError

        val canMergeJumpsWithDefaultExit = getCommonSuccessorIfAny(defaultExits) == jumpTarget
        val conditional = !canMergeJumpsWithDefaultExit && defaultExits.isNotEmpty()
        val elements = jumpExits.map { it.element as JetExpression }
        val elementToInsertAfterCall = if (canMergeJumpsWithDefaultExit) null else elements.first()
